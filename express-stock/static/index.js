...
function changeGrowthPresentation(el) {
  ...
  else el.innerHTML = el.dataset.change;
}
...
const addCompaniesToList = companies => {
  ...
  companies.forEach(company => {
    ...
    const growthButtonClasses =
      "stockGain" + (company.changePercent < 0 ? " negative" : "");
    ...
    companyContainer.innerHTML = `<span class="companyName">${company.symbol +
      ` (${company.companyName})`}</span>
                                  <div class="companyInfoContainer">
                                    <span class="stockPrice">${company.price.toFixed(
                                      2
                                    )}</span>
...
async function loadStocks(companies) {
  const url =
    "http://localhost:3000/api/v1/prices?company=" + companies.join(",");
